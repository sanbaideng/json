...
//初始化区域图的对象
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		e.data.inthis.addNode(e.data.inthis.$id+"_node_"+e.data.inthis.$max,{id:e.data.inthis.$id+"_node_"+e.data.inthis.$max,name:"node_"+e.data.inthis.$max,left:X,top:Y,type:e.data.inthis.$nowType});
		e.data.inthis.$max++;
	  });
	  //划线时用的绑定
...
GooFlow.prototype = {
    ...
    addNode: function (id, json) {//yangyang
        ...
        var ss= {nodeid:id,name:json.name,left:json.left,top:json.top,type:json.type,width:json.width,height:json.height,alt:json.alt};
        this.$nodeData[id] = ss; ////////////////////////////yangyang
        alert(ss);
        //{id:e.data.inthis.$id+"_node_"+e.data.inthis.$max,name:"node_"+e.data.inthis.$max,left:X,top:Y,type:e.data.inthis.$nowType}
        ++this.$nodeCount;
        if (this.$editable) {
...
GooFlow.prototype = {
    ...
    exportData: function () {
    	...
    	var nodes_s = this.$nodeData;
    	var myint = 0;
    	for (var k1 in nodes_s) {
    		 mynode[myint]=nodes_s[k1];
    		 if(!nodes_s[k1].marked){delete nodes_s[k1]["marked"];}
    		 myint++;
    	};

    	var myline = new Array();
    	var lines_s = this.$lineData;
    	var mylineint = 0;
    	for (var k1 in lines_s) {
    		 myline[mylineint]=lines_s[k1];
    		 if(!lines_s[k1].marked){delete lines_s[k1]["marked"];}
    		 mylineint++;
    	};

        var ret = { title: this.$title, nodes: mynode, lines: myline, areas: this.$areaData, initNum: this.$max };
        debugger;
        /*for (var k1 in ret.nodes) {
            if (!ret.nodes[k1].marked) {
                delete ret.nodes[k1]["marked"];
    ...
    addLine: function (id, json) {
        ...
        this.$lineData[id].to = json.to;
        this.$lineData[id].name = json.name;
        this.$lineData[id].lineid = id;//yangyang
        if (json.mark) this.$lineData[id].marked = json.mark;
        else this.$lineData[id].marked = false;
